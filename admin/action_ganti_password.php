...
if($_POST['submit']){
		...
		$cek 			= $koneksi->query("SELECT `password` FROM pendaftaran WHERE `password`='$password'");
		...
		if($cek->num_rows){
			if(strlen($password_baru) >= 5){
				...
				if($password_baru == $konfirmasi_password){
					...
					$user_id		= $_SESSION['id_pendaftaran']; //ini dari session saat login
					
					$update 		= $koneksi->query("UPDATE pendaftaran SET `password`='$password_baru' WHERE `id_pendaftaran`='$user_id'");
					if($update){
